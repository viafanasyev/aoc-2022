...
func parseTest(line string) func(int) bool {
    ...
    return func(x int) bool { return x % value == 0 }
}
...
func parseMonkey(lines []string) monkey {
    ...
    return monkey { items, operation, test, monkeyIfTrue, monkeyIfFalse, 0 }
}

func makeMonkeyPlay(m *monkey, monkeys []monkey) {
    for !m.items.IsEmpty() {
...
func main() {
    ...
    for round := 0; round < rounds; round++ {
        for i := 0; i < len(monkeys); i++ {
            makeMonkeyPlay(&monkeys[i], monkeys)
        }
    }
