...
func main() {
    ...
    spaceToFree := requiredDiskSpace - (totalDiskSpace - fileToTotalSize[root])

    resultSize := fileToTotalSize[root]
    resultDir := root
    for file, totalSize := range fileToTotalSize {
        if file.isDir() && totalSize >= spaceToFree && totalSize < resultSize {
            resultSize = totalSize
            resultDir = file
        }
    }
    fmt.Printf("Answer: dir = %s, size = %d \n", resultDir.pwd(), resultSize)
}
