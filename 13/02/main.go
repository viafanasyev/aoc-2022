...
func comparePacketPair(firstPacket packet, secondPacket packet) int {
...
func main() {
    ...
    nonEmptyLines := utils.Filter(lines, func(line string) bool { return len(line) > 0 })
    packets := utils.Map(nonEmptyLines, parsePacket)

    firstDelimeter := packet { makeListNotInt([]listOrInt { makeListNotInt([]listOrInt { makeIntNotList(2) }) }) }
    secondDelimeter := packet { makeListNotInt([]listOrInt { makeListNotInt([]listOrInt { makeIntNotList(6) }) }) }
    packets = append(packets, firstDelimeter)
    packets = append(packets, secondDelimeter)

    sort.Slice(packets, func(i, j int) bool { return comparePacketPair(packets[i], packets[j]) > 0 })

    firstDelimeterPos := 1 + utils.IndexOf(packets, func (p packet) bool {
        return isListNotInt(p.data) &&
                len(p.data.listValue) == 1 &&
                isListNotInt(p.data.listValue[0]) &&
                len(p.data.listValue[0].listValue) == 1 &&
                isIntNotList(p.data.listValue[0].listValue[0]) &&
                p.data.listValue[0].listValue[0].intValue == 2
    })
    secondDelimeterPos := 1 + utils.IndexOf(packets, func (p packet) bool {
        return isListNotInt(p.data) &&
                len(p.data.listValue) == 1 &&
                isListNotInt(p.data.listValue[0]) &&
                len(p.data.listValue[0].listValue) == 1 &&
                isIntNotList(p.data.listValue[0].listValue[0]) &&
                p.data.listValue[0].listValue[0].intValue == 6
    })
    fmt.Printf("First delimeter position: %d\n", firstDelimeterPos)
    fmt.Printf("Second delimeter position: %d\n", secondDelimeterPos)
    fmt.Printf("Answer: %d\n", firstDelimeterPos * secondDelimeterPos)
}
